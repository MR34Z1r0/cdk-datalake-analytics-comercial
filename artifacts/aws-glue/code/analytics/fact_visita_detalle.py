...
try:
    df_t_visita = read_table("t_visita", S3_PATH_DOM)
    df_t_avance_dia = read_table("t_avance_dia", S3_PATH_DOM)
    
    df_fact_visita_detalle_com = get_table("fact_visita_detalle", S3_PATH_COM).where(
...
# CREATE
try:
    tmp_fact_visita_detalle = (
        df_t_visita.alias("v")
        .join(
            df_t_avance_dia.alias("ad"),
            (col("v.id_avance_dia") == col("ad.id_avance_dia")),
            "left",
        )
        .where(
            (col('v.id_pais').isin(cod_pais))
            & (col('v.id_periodo').isin(PERIODOS))
            & (col('v.es_eliminado') == 0)
        )
        .select(
            col("v.id_pais"),
            col("v.id_periodo"),
            col("ad.id_avance_dia"),
            col("ad.id_vendedor"),
            col("ad.id_sucursal"),
            col("ad.id_modelo_atencion"),
            col("ad.cod_avance_dia"),
            col("ad.cod_unidad"),
            col("ad.cod_modulo"),
            col("ad.desc_modulo"),
            col("ad.cod_ruta"),
            ...
            col("ad.cod_zona"),
            col("ad.desc_zona"),
            col("ad.fecha_avance_dia"),
            col("ad.fecha_inicio").alias('fecha_inicio_avance'),
            col("ad.fecha_fin").alias('fecha_fin_avance'),
            col("ad.coordx_inicio"),
            col("ad.coordx_fin"),
            col("ad.coordy_inicio"),
            col("ad.coordy_fin"),
            col("ad.distancia_recorrida"),
            col("ad.cant_cuota_cambio"),
            col("ad.es_eliminado"),
            col("v.id_visita"),
            col("v.id_cliente"),
            col("v.id_visita_anterior"),
            col("v.cod_visita"),
            col("v.fecha_visita"),
            col("v.fecha_inicio").alias('fecha_inicio_visita'),
            col("v.fecha_fin").alias('fecha_fin_visita'),
            col("v.tiempo_visita_transcurrido"),
            col("v.secuencia"),
            col("v.es_activo"),
            col("v.es_programado"),
            col("v.es_pedido_sugerido"),
            col("v.es_visita_auto"),
            col("v.desc_motivo_no_pedido"),
            col("v.coordx"),
            col("v.coordy"),
            col("v.distancia_fin"),
            col("v.tiempo_visita"),
            col("v.fecha_check_in"),
            col("v.fecha_check_out"),
            col("v.es_check_out_automatico"),
            col("v.es_pedido_levantado"),
            col("v.cant_total_cajas"),
            col("v.cant_total_cajas_fisicas"),
            col("v.imp_total"),
            col("v.cant_total_litros"),
            col("v.cant_total_pedidos"),
            col("v.cant_total_skus"),
            col("v.cant_total_unidades"),
            col("v.cant_total_unidades_cambio")
        )
    )
except Exception as e:
    logger.error(str(e))
...
# DELETE
try:
    df_fact_visita_detalle_com = df_fact_visita_detalle_com.where(
...
# INSERT
try:
    df_fact_visita_detalle_com = df_fact_visita_detalle_com.union(tmp_fact_visita_detalle)
except Exception as e:
    logger.error(str(e))
...
# SAVE
try:
    table_name = "fact_visita_detalle"
    df = df_fact_visita_detalle_com
